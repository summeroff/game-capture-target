...
// clang-format off
// No vertex/fragment SPIR-V. Vulkan's default visual is a cycling clear colour
// plus a title-bar frame counter (enough for Game Capture freeze-tell).
// Draw-list scenes (--scene aurora/orbital/…) are d3d11/d3d12/GDI only.
// clang-format on
...
public:
  ...
  bool Init(HWND hwnd, const Config& cfg, std::wstring* error) override
  {
    ...
    Log("vulkan: native visual = cycling clear + title frame counter");
    return true;
  }
